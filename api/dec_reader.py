...
def force_metro_emojis():
    for ms in NEAREST_METRO_STATIONS.values():
        ...
        ), f"Линия {ms['line']} используется универом, а эмодзи нет"
    ...
    def generate_ical(self, tt):
        ...
        for name in ["NAME", "X-WR-CALNAME"]:
            cal.extra.append(
                ContentLine(
                    name,
                    value=f"Расписание {tt['info']['group']['name']} (8am.octonezd.me)",
                )
            )
        ...
        cal.extra.append(
            ContentLine(
                "URL",
                value=f"webcal://8am.octonezd.me/group/{tt['info']['group']['groupID']}.ics",
            )
        )
        debug_info = Event(
        ...
        for lesson in tt["rasp"]:
            ...
            event.extra.append(
                ContentLine("X-8AM-TEACHER", value=lesson["преподаватель"])
            )
            if "-" in lesson["аудитория"]:
                location_id, room = lesson["аудитория"].split("-")
                event.extra.append(ContentLine("X-8AM-ROOM", value=room))
                if location_id in LOCATIONS:
                    ...
                    event.extra.append(
                        ContentLine("X-8AM-LOCATION", value=LOCATIONS[location_id])
                    )
                    event.extra.append(
                        ContentLine("X-8AM-METRO", value=nearest_metro["station"])
                    )
                    event.extra.append(
                        ContentLine("X-8AM-METRO-LINE", value=nearest_metro["line"])
                    )
                else:
                    location = f"Хуй знает где, каб. {room}"
                    event.extra.append(
                        ContentLine("X-8AM-LOCATION", value="Неизвестно")
                    )
            else:
